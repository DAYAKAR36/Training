...
void main() 
{
    ...
    char command[200];
    // sprintf(command, "wget -q -o %s \"https://api.openweathermap.org/data/2.5/weather?q=%s&appid=35fcf43fe03b83f3ee39a112e3a64194&units=metric\"", FILE_NAME, city_name);
    sprintf(command, "curl -s \"https://api.openweathermap.org/data/2.5/weather?q=%s&appid=35fcf43fe03b83f3ee39a112e3a64194&units=metric\" > %s", city_name, FILE_NAME);
    system(command);
    char weather_details[1000];
    FILE *fp_weather = fopen(FILE_NAME, "r");
